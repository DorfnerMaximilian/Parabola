...
class bond:
    def __init__(
        self, atom_index_1, atom_index_2, bond_index, distance=None, order=None
    ):
...
def get_adjacency_matrix(coordinates, atomicsymbols, tol=0.4):
    ...
    for i in range(n):
        for j in range(i + 1, n):
            r_sum = (
                covalent_radii.get(atomicsymbols[i], 0.7)
                + covalent_radii.get(atomicsymbols[j], 0.7)
                + tol
            )
        ...
        for j in range(i + 1, n):  # upper triangle only (undirected graph)
            if adjacency_matrix[i][j]:
                distance = np.linalg.norm(
                    self.atoms[j].coordinates - self.atoms[i].coordinates
                )
...
def decode_sssr_vectors(self, sssr_basis):
    ...
    for edge_tuple, bond_obj in self.bonds.items():
        ...
        if bond_obj.bond_index not in index_to_edge:
            index_to_edge[bond_obj.bond_index] = (
                bond_obj.atom_index_1,
                bond_obj.atom_index_2,
            )
    ...
    for vec in sssr_basis:
        ...
        for idx in edge_indices:
            ...
            if u not in cycle_adj:
                cycle_adj[u] = []
            if v not in cycle_adj:
                cycle_adj[v] = []
...
def determine_bond_order(self):
    ...
    for atom in self.atoms:
        if len(atom.adjacent) == 2:
            ...
            if (
                type(self.bonds[(idx, idx_ad_1)].bond_order) != None
                and type(self.bonds[(idx, idx_ad_2)].bond_order) == None
            ):
                self.bonds[(idx, idx_ad_2)].bond_order = (
                    octettrules[atomtype] - self.bonds[(idx, idx_ad_1)].bond_order
                )
                self.bonds[(idx_ad_2, idx)].bond_order = (
                    octettrules[atomtype] - self.bonds[(idx, idx_ad_1)].bond_order
                )
            elif (
                type(self.bonds[(idx, idx_ad_2)].bond_order) != None
                and type(self.bonds[(idx, idx_ad_1)].bond_order) == None
            ):
                self.bonds[(idx, idx_ad_1)].bond_order = (
                    octettrules[atomtype] - self.bonds[(idx, idx_ad_2)].bond_order
                )
                self.bonds[(idx_ad_1, idx)].bond_order = (
                    octettrules[atomtype] - self.bonds[(idx, idx_ad_2)].bond_order
                )
            else:
                ...
                if rem_valence > 0 and rem_valence % 2 == 0:
                    ...
                    if self.atoms[idx_ad_1].symbol == self.atoms[idx_ad_2].symbol:
                        ...
                        raise ValueError(
                            f"Cannot determine bond order for atom {idx} ({atom.symbol}): "
    ...
    for atom in self.atoms:
        if len(atom.adjacent) == 3:
            ...
            if rem_valence < 0:
                raise ValueError(
                    f"Atom {idx} ({atomtype}) is overbonded: assigned more than allowed valence."
                )
            ...
            if len(unknown_bonds) == 0:
                ...
                if rem_valence % 2 == 0:
                    ...
                    self.atoms[unknown_bonds[0]].symbol
                    == self.atoms[unknown_bonds[1]].symbol
                ):
                    bond_order_to_assign = rem_valence / 2.0
                    # Assign order to the first bond and update neighbor
                    self.bonds[(idx, unknown_bonds[0])].bond_order = (
                        bond_order_to_assign
                    )
                    self.bonds[(unknown_bonds[0], idx)].bond_order = (
                        bond_order_to_assign
                    )
                    self.atoms[unknown_bonds[0]].valence -= bond_order_to_assign

                    # Assign order to the second bond and update neighbor
                    self.bonds[(idx, unknown_bonds[1])].bond_order = (
                        bond_order_to_assign
                    )
                    self.bonds[(unknown_bonds[1], idx)].bond_order = (
                        bond_order_to_assign
                    )
                    ...
                    raise ValueError(
                        f"Cannot evenly assign bond orders to two unknown bonds for atom {idx} ({atomtype})."
                    )
                ...
                if rem_valence % 3 == 0:
                    ...
                    raise ValueError(
                        f"Cannot evenly assign bond orders to three unknown bonds for atom {idx} ({atomtype})."
                    )


def valence_one_modifications(
    original_graph: MolecularGraph, type="F", target_index=0
) -> MolecularGraph:
