...
return matrix_element;
=======
>>>>>>> fc9c751 (formatting c++ with clang-format -style=llvm -i CPP_Extension/src/AtomicBasis.cpp)
  }

  return sum;
}
double KFunction(const std::array<double, 3> &Y1,
                 const std::array<double, 3> &Y2, const std::array<int, 3> &iis,
...
std::complex<double> Kqcomponent(double Y1k, double Y2k, int ik, int jk,
...
std::complex<double> KqFunction(const std::array<double, 3> &Y1,
...
std::complex<double> IJ_q_Int(const std::array<double, 3> &R1,
                              const std::array<double, 3> &R2,
                              const std::vector<Monomial> &lm1,
...
double IJ_Int(const std::array<double, 3> &X, const std::vector<Monomial> &lm1,
...
double IJ_Int_momentum(const std::array<double, 3> &X,
                       const std::vector<Monomial> &lm1,
                       const std::vector<Monomial> &lm2, double A1, double A2,
                       int modus) {
  // computes the IJ integral using the monomial decomposition of the
        ...
        matrix_element +=
            contr_coeff1[it1] * contr_coeff2[it2] *
                IJ_Int({R1[0] - R2_shifted[0], R1[1] - R2_shifted[1],
                        R1[2] - R2_shifted[2]},
                       lm1, lm2, alphas1[it1], alphas2[it2], direction) +
            contr_coeff1[it1] * contr_coeff2[it2] * (R1[direction - 1]) *
                IJ_Int({R1[0] - R2_shifted[0], R1[1] - R2_shifted[1],
                        R1[2] - R2_shifted[2]},
                       lm1, lm2, alphas1[it1], alphas2[it2], 0);
      }
...
// Function to compute the overlap of two basis functions
std::complex<double> get_phase_Matrix_Element(
    const std::array<double, 3> &R1, const std::vector<double> &contr_coeff1,
    const std::vector<double> &alphas1, const std::vector<Monomial> &lm1,
    const std::array<double, 3> &R2, const std::vector<double> &contr_coeff2,
    const std::vector<double> &alphas2, const std::vector<Monomial> &lm2,
...
double *get_T_Matrix(const char *atoms_set1[], const double positions_set1[],
                     const double alphas_set1[], const int alphasLengths_set1[],
                     const double contr_coef_set1[],
                     const int contr_coefLengths_set1[], const char *lms_set1[],
                     int size_set1, const char *atoms_set2[],
                     const double positions_set2[], const double alphas_set2[],
                     const int alphasLengths_set2[],
                     const double contr_coef_set2[],
                     const int contr_coefLengths_set2[], const char *lms_set2[],
                     int size_set2, const double cell_vectors[],
                     int size_cell_vectors) {

  // Initialize the map of monomials
  std::unordered_map<std::string, std::vector<Monomial>> solidHarmonics =
      getSolidHarmonics();
  // Process the cell_vectors
  std::vector<double> cell_vectors_vec(cell_vectors,
                                       cell_vectors + size_cell_vectors);
  ...
  for (int i = 0; i < size_set1; ++i) {
    ...
    std::vector<double> alphas_list_set1(alphas_set1 + alphasIndex_set1,
                                         alphas_set1 + alphasIndex_set1 +
                                             alphasLength_set1);
    ...
    std::vector<double> contr_coef_list_set1(
        contr_coef_set1 + contrCoefIndex_set1,
        contr_coef_set1 + contrCoefIndex_set1 + contrCoefLength_set1);
    contrCoefIndex_set1 += contrCoefLength_set1;

    std::string lm(lms_set1[i]);

    Basisfunction basisfunction(atom, position, alphas_list_set1,
                                contr_coef_list_set1, lm);
    basisfunctions_set1.push_back(basisfunction);

    arrayIndex_set1 +=
        3; // Increment by 3 for positions (3 elements per position)
  }
  ...
  for (int i = 0; i < size_set2; ++i) {
    ...
    std::vector<double> alphas_list_set2(alphas_set2 + alphasIndex_set2,
                                         alphas_set2 + alphasIndex_set2 +
                                             alphasLength_set2);
    ...
    std::vector<double> contr_coef_list_set2(
        contr_coef_set2 + contrCoefIndex_set2,
        contr_coef_set2 + contrCoefIndex_set2 + contrCoefLength_set2);
    contrCoefIndex_set2 += contrCoefLength_set2;

    std::string lm(lms_set2[i]);

    Basisfunction basisfunction(atom, position, alphas_list_set2,
                                contr_coef_list_set2, lm);
  ...
  for (int i = 0; i < size_set1; ++i) {
    for (int j = 0; j < size_set2; ++j) {
      double overlap = getoverlap(
          basisfunctions_set1[i].position, basisfunctions_set1[i].contr_coef,
          basisfunctions_set1[i].alphas,
          solidHarmonics[basisfunctions_set1[i].lm],
          basisfunctions_set2[j].position, basisfunctions_set2[j].contr_coef,
          basisfunctions_set2[j].alphas,
          solidHarmonics[basisfunctions_set2[j].lm], cell_vectors_vec);
...
double *get_WFN_On_Grid(const double xyzgrid[], int size_xyzgrid,
                        const double WFNcoefficients[], const char *atoms_set[],
                        const double positions_set[], const double alphas_set[],
                        const int alphasLengths_set[],
                        const double contr_coef_set[],
                        const int contr_coefLengths_set[],
                        const char *lms_set[], int size_set,
                        const double cell_vectors[], int size_cell_vectors) {
  ...
  std::vector<double> WFNcoefficients_vec(WFNcoefficients,
                                          WFNcoefficients + size_set);
  ...
  for (int i = 0; i < size_set; ++i) {
    ...
    std::vector<double> alphas_list_set(alphas_set + alphasIndex_set,
                                        alphas_set + alphasIndex_set +
                                            alphasLength_set);
    alphasIndex_set += alphasLength_set;

    // Extract contr_coef for the current basis function in set 1
    int contrCoefLength_set = contr_coefLengths_set[i];
    std::vector<double> contr_coef_list_set(
        contr_coef_set + contrCoefIndex_set,
        contr_coef_set + contrCoefIndex_set + contrCoefLength_set);
    contrCoefIndex_set += contrCoefLength_set;

    std::string lm(lms_set[i]);

    Basisfunction basisfunction(atom, position, alphas_list_set,
                                contr_coef_list_set, lm);
...
#pragma omp parallel for
  for (int i = 0; i < size_xyzgrid; i += 3) {
    double WFNvalue = 0.0;
    const std::array<double, 3> &r = {xyzgrid_vec[i], xyzgrid_vec[i + 1],
                                      xyzgrid_vec[i + 2]};
    for (int j = 0; j < size_set; ++j) {
      WFNvalue +=
          WFNcoefficients_vec[j] *
...
double *get_Local_Potential_On_Grid(
    const double xyzgrid[], int size_xyzgrid, const double MatrixElements[],
    const char *atoms_set[], const double positions_set[],
    const double alphas_set[], const int alphasLengths_set[],
    const double contr_coef_set[], const int contr_coefLengths_set[],
    const char *lms_set[], int size_set, const double cell_vectors[],
    int size_cell_vectors) {
  ...
  std::vector<double> xyzgrid_vec(xyzgrid, xyzgrid + size_xyzgrid);
  // Process the WFNcoefficients
  std::vector<double> MatrixElements_vec(MatrixElements,
                                         MatrixElements + size_set * size_set);
  // Process the input arrays - construct Basisfunction instances
  std::vector<Basisfunction> basisfunctions_set;
  int arrayIndex_set = 0;
  int alphasIndex_set = 0;
    ...
    for (int j = 0; j < size_set; ++j) {
      for (int k = 0; k < size_set; ++k) {
        Local_Potential_Value +=
            MatrixElements_vec[j * size_set + k] *
            getBasisFunctionOnGrid(
                r, basisfunctions_set[j].position,
                basisfunctions_set[j].contr_coef, basisfunctions_set[j].alphas,
                solidHarmonics[basisfunctions_set[j].lm], cell_vectors_vec) *
...
double *get_Position_Operators(
    const char *atoms_set1[], const double positions_set1[],
    const double alphas_set1[], const int alphasLengths_set1[],
    const double contr_coef_set1[], const int contr_coefLengths_set1[],
    const char *lms_set1[], int size_set1, const char *atoms_set2[],
    const double positions_set2[], const double alphas_set2[],
    const int alphasLengths_set2[], const double contr_coef_set2[],
    const int contr_coefLengths_set2[], const char *lms_set2[], int size_set2,
    const double cell_vectors[], int size_cell_vectors, int direction) {
...
#pragma omp parallel for collapse(2)
  for (int i = 0; i < size_set1; ++i) {
    for (int j = 0; j < size_set2; ++j) {
      double overlap = get_p_Matrix_Element(
          basisfunctions_set1[i].position, basisfunctions_set1[i].contr_coef,
          basisfunctions_set1[i].alphas,
          solidHarmonics[basisfunctions_set1[i].lm],
          basisfunctions_set2[j].position, basisfunctions_set2[j].contr_coef,
...
=======
std::complex<double> *get_Phase_Operators(
    const char *atoms_set1[], const double positions_set1[],
    const double alphas_set1[], const int alphasLengths_set1[],
    const double contr_coef_set1[], const int contr_coefLengths_set1[],
    const char *lms_set1[], int size_set1, const char *atoms_set2[],
    ...
    const int alphasLengths_set2[], const double contr_coef_set2[],
    const int contr_coefLengths_set2[], const char *lms_set2[], int size_set2,
    const double cell_vectors[], int size_cell_vectors, const double q[]) {
>>>>>>> fc9c751 (formatting c++ with clang-format -style=llvm -i CPP_Extension/src/AtomicBasis.cpp)
  ...
  std::complex<double> *OLPasArray_ptr =
      new std::complex<double>[size_set1 * size_set2];
// Now, loop over both sets and compute overlaps
#pragma omp parallel for collapse(2)
<<<<<<< HEAD
for (int i = 0; i < size_set1; ++i) {
...
=======
  for (int i = 0; i < size_set1; ++i) {
    for (int j = 0; j < size_set2; ++j) {
      std::complex<double> matrix_element = get_phase_Matrix_Element(
          basisfunctions_set1[i].position, basisfunctions_set1[i].contr_coef,
          basisfunctions_set1[i].alphas,
          solidHarmonics[basisfunctions_set1[i].lm],
          basisfunctions_set2[j].position, basisfunctions_set2[j].contr_coef,
