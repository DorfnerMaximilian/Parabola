...
double JInt(const std::array<double, 3>& X, const std::vector<Monomial>& lm1,const std::vector<Monomial>& lm2, double A1, double A2) {
    ...
    for (const auto& Z1 : lm1) {
        for (const auto& Z2 : lm2) {
            sum += Z1.prefactor * Z2.prefactor * KFunction(Y1, Y2, Z1.indices, Z2.indices, alpha);
        }
    }
return sum;
}
// Function to compute the I integral
double IInt(const std::array<double,3>& X,
            const std::vector<Monomial>& lm1, const std::vector<Monomial>& lm2,
            double A1, double A2) {
    
    double Jintegral = JInt(X, lm1, lm2, A1, A2);
        ...
        for (size_t it1 = 0; it1 < alphas1.size(); ++it1) {
            for (size_t it2 = 0; it2 < alphas2.size(); ++it2) {
                overlap += contr_coeff1[it1] * contr_coeff2[it2] *
                           IInt({R1[0] - R2_shifted[0], R1[1] - R2_shifted[1], R1[2] - R2_shifted[2]},
                                lm1, lm2, alphas1[it1], alphas2[it2]);
            }
        }
    }
...
void freeOLPasArray_ptr(double* array) {
