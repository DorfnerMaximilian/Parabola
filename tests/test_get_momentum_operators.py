...
class Test_Momentum_Operators(unittest.TestCase):
    def test_OBC(self):
        print("\nTesting parabola's", "\033[1m", "get_momentum_operators", "\033[0m")
        print("Started OBC Test")
        regression_data = np.load("data/OBC/momentum_operators.npy")
        Atoms = p.Read.read_atomic_coordinates("data/OBC/Benzene_opt.xyz")
        Basis = p.atomic_basis.getBasis("data/OBC/")
        parabola_data = p.atomic_basis.get_momentum_operators(Atoms, Basis)
        np.testing.assert_array_almost_equal(parabola_data, regression_data, 13, "OBC Test failed!")

    def test_PBC(self):
        print("\nStarted PBC Test")
        regression_data = np.load("data/PBC/momentum_operators.npy")
        ...
        parabola_data = p.atomic_basis.get_momentum_operators(Atoms, Basis, cellvectors)
