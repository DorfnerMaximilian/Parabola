...
def commute_check(mol, path='./'):
    ...
    # Or use the DFT output ksh = mol.electronic_structure.KS_Hamiltonian_alpha
...
def bloch_from_old(mol,path='./'):
    ...
    olm = mol.electronic_structure.OLM
    olmm1w = mol.electronic_structure.inverse_sqrt_OLM
                ...
                if periodic_bool[d]:
                    ...
                    for block in blocks:
                        ...
                        if size == 1:
                            ...
                            this_block_bloch = np.conjugate(par_bloch_states[i: i + size + 1, :]) @ olm @ T_op @ par_bloch_states[i: i + size + 1, :].T
                            val, vec = np.linalg.eig(this_block_bloch)
                            if np.max(np.abs(np.conjugate(vec).T @ vec - np.eye(vec.shape[0]))) > 1e-5:
                                print('bloch transformation not unitary', np.max(np.abs(np.conjugate(vec).T @ vec - np.eye(vec.shape[0]))))
    ...
    if already_stored_flag:
        ...
        for d in [0, 1, 2]:
            if periodic_bool[d]:
                ...
                time1 = time.time()
                phase = AtomicBasis.get_phase_operators(atoms, basis, q_vector=unit_vectors[d], cell_vectors=cellvectors, cutoff_radius=25)
    ...
    if nh > 0:
        ...
        for ind in occ_ind:
            if estate_dict[ind][0] in prim_gam_sym:
                ...
                energies.append(mol.electronic_structure.ElectronicEigenstates["alpha"][estate_dict[ind][0]][int(estate_dict[ind][1])].energy)
    ...
    for k_state1 in k_blochs[point1]:
        for k_state2 in k_blochs[point2]:
            ...
            if -5.5 > energy1 * 27.211 > -10.5 and -5.5 > energy2 * 27.211 > -10.5:
                ...
                print(np.conjugate(k_state1.T) @ over_mat2 @ k_state2,np.abs(np.conjugate(k_state1.T) @ over_mat2 @ k_state2))

    return None

debug_q_path1 = [[np.array((0.0,0.0,0.0)),np.array((0.2,0.0,0.0)),0,+1],[np.array((0.0,0.0,0.0)),np.array((-0.2,0.0,0.0)),0,-1],[np.array((0.2,0.0,0.0)),np.array((0.4,0.0,0.0)),0,1],[np.array((-0.2,0.0,0.0)),np.array((-0.4,0.0,0.0)),0,-1]]
debug_q_path2 = [[np.array((0.0,0.0,0.0)),np.array((0.0,0.2,0.0)),1,+1],[np.array((0.0,0.0,0.0)),np.array((0.0,-0.2,0.0)),1,-1],[np.array((0.0,0.2,0.0)),np.array((0.0,0.4,0.0)),1,+1],[np.array((0.0,-0.2,0.0)),np.array((0.0,-0.4,0.0)),1,-1]]
debug_q_path3 = [[np.array((0.2,0.0,0.0)),np.array((0.2,0.2,0.0)),1,+1],[np.array((0.2,0.2,0.0)),np.array((0.4,0.2,0.0)),0,+1],[np.array((0.4,0.2,0.0)),np.array((0.4,0.4,0.0)),1,+1],[np.array((-0.2,0.0,0.0)),np.array((-0.2,-0.2,0.0)),1,-1],[np.array((-0.2,-0.2,0.0)),np.array((-0.4,-0.2,0.0)),0,-1],[np.array((-0.4,-0.2,0.0)),np.array((-0.4,-0.4,0.0)),1,-1]]
debug_q_path4 = [[np.array((0.2,0.0,0.0)),np.array((0.2,-0.2,0.0)),1,-1], [np.array((0.2,-0.2,0.0)),np.array((0.4,-0.2,0.0)),0,+1], [np.array((0.4,-0.2,0.0)),np.array((0.4,-0.4,0.0)),1,-1],[np.array((-0.2,0.0,0.0)),np.array((-0.2,0.2,0.0)),1,1],[np.array((-0.2,0.2,0.0)),np.array((-0.4,0.2,0.0)),0,-1],[np.array((-0.4,0.2,0.0)),np.array((-0.4,0.4,0.0)),1,1]]
debug_q_path5 = [[np.array((0.2,0.2,0.0)),np.array((0.2,0.4,0.0)),1,+1],[np.array((-0.2,-0.2,0.0)),np.array((-0.2,-0.4,0.0)),1,-1],[np.array((0.2,-0.2,0.0)),np.array((0.2,-0.4,0.0)),1,-1],[np.array((-0.2,0.2,0.0)),np.array((-0.2,0.4,0.0)),1,+1]]
...
def band_index(mol, nh, nl, name_of_bloch_file='general', name_of_k_res_bloch='k_resolved_blochs.pickle', path='./'):
    ...
    all_connected_bands = []
    bloch_states_full = testing_bloch(mol,path=path, name_of_file=name_of_bloch_file)#bloch_form(mol, path=path, name_of_file=name_of_bloch_file)
    ...
    for state in prim_gam_states:
        ...
        ene = mol.electronic_structure.ElectronicEigenstates["alpha"][state[0]][int(state[1])].energy
    ...
    for state in prim_blochs:
        ...
        for q_index in range(0, len(q_path)):
            ...
            if not np.any(np.all(np.isclose(np.array(calculated_q_points), current_qpoint, atol=1e-7), axis=1)):
                ...
                print(np.round(cart_to_direct_dict[tuple(np.round(previous_qpoint, 6))],3),np.round(cart_to_direct_dict[tuple(np.round(current_qpoint, 6))],3), sortedover[-5:], np.real(np.conjugate(previous_state.T)@ksh@previous_state) *27.211 , np.real(np.conjugate(max_state.T)@ksh@max_state) *27.211)
...
def old_band_index(mol, nh, nl, name_of_bloch_file='general', path='./'):
    ...
    for number in range(-5, 5):
        ...
        this_state_en = mol.electronic_structure.ElectronicEigenstates["alpha"][this_state[0]][
            int(this_state[1])].energy
    ...
    for state in prim_gam_states:
        prim_blochs.append(mol.electronic_structure.ElectronicEigenstates["alpha"][state[0]][int(state[1])].a)
        print(mol.electronic_structure.ElectronicEigenstates["alpha"][state[0]][int(state[1])].energy)
    ...
    for state in prim_blochs:
        ...
        for q_index in range(1, len(q_arrays)):
            ...
            phase = AtomicBasis.get_phase_operators(atoms, basis, q_vector=current_qpoint, cell_vectors=cellvectors, cutoff_radius=25)
            calculated_q_points.append(previous_qpoint)
            previous_state = band[tuple(previous_qpoint)]
            third = time.time()
            if not np.any(np.all(np.isclose(np.array(calculated_q_points), current_qpoint, atol=1e-7), axis=1)):
...
def wan_real_plot(mol, mode, ind=[0], Wan_npy='Wannier_orbitals_occ.npy', path='./', N1=100, N2=100, N3=100):
    if mode.casefold() == 'q':
        ...
        periodic = periodic_bool.astype(int)
        cellvectors = Geometry.getNeibouringCellVectors(cell=mol.cellvectors, m=periodic[0], n=periodic[1], l=periodic[2])  # assuming the supercell is large enough so that just the first neighbouring cells are enough; otherwise a convergence check would be needed!
        data , gridx, gridy, gridz, atoms = TDDFT.WFNsOnGrid(ids=ind, N1=N1, N2=N2, N3=N3, cell_vectors=cellvectors, wannier_printflag=True, Wan_file=Wan_npy, saveflag=False, parentfolder=path)
...
def wan_interpolate_bandstruc(mol, special_k_points, Wan_file='Wannier_orbitals8lumobands.npy', path='./'):
    ...
    for d1, d2 in [[0, 1], [1, 2], [0, 2]]:
        if periodic_bool[d1] and periodic_bool[d2]:
            T_op1 = mol.electronic_structure.Electronic_Symmetry.Symmetry_Generators['t' + str(d1 + 1)]
            T_op2 = mol.electronic_structure.Electronic_Symmetry.Symmetry_Generators['t' + str(d2 + 1)]
    ...
    T_opa = mol.electronic_structure.Electronic_Symmetry.Symmetry_Generators['t1']
    T_opb = mol.electronic_structure.Electronic_Symmetry.Symmetry_Generators['t2']
