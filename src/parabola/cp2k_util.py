...
def get_cp2k_exec(cluster="local"):
    if cluster == "slurm":
        ...
        if cp2k_exec is None:
            raise RuntimeError(
                "cp2k.psmp not found in PATH, did you load the CP2K module in your jobscript?"
            )
        return cp2k_exec
    else:  # local
        ...
        if pathtocp2k is None:
            raise RuntimeError(
                "Environment variable cp2kpath not set for local execution"
            )
...
def write_pos_file(
    ...
    with open(path, "a") as f:
        ...
        for i in range(3):
            f.write(
                f"{cell_bohr[i,0]:.12f} {cell_bohr[i,1]:.12f} {cell_bohr[i,2]:.12f}\n"
            )
        f.write("End Cell\n")
        ...
        if stress_Ha_bohr_3 is not None and len(stress_Ha_bohr_3) == 3:
            ...
            for i in range(3):
                f.write(
                    f"{stress_Ha_bohr_3[i,0]:.12f} {stress_Ha_bohr_3[i,1]:.12f} {stress_Ha_bohr_3[i,2]:.12f}\n"
                )
            f.write("End Stress\n")
...
def write_geo_log_file(name, atoms, coordinates_bohr, energy_Ha, step, path):
    ...
    with open(path, "a") as f:
        ...
        f.write(
            f"Optimization of {name} | Step: {step} | Energy[Ha]={energy_Ha:.12f}\n"
        )
        for it, coord in enumerate(coordinates_bohr):
...
def geo_opt(
    ...
    forces_Ha_bohr = (
        trans_projector @ rot_projector @ np.array(forces_Ha_bohr).flatten()
    )
    force_history = []
    ...
    bonds, angles, dihedrals, impropers = generate_internal_representation(
        atomic_symbols=atoms, coordinates_bohr=coordinates_bohr, cell_bohr=cell_bohr
    )
    internals = (bonds, angles, dihedrals, impropers)

    print(
        f"{it:4d} | {energy_Ha:12.8f} | {'--------':>12} | {'--------':>12} | "
        f"{drm_force:10.6f} | {force_max:10.6f} | {'--------':>10} | {0.0:8.1f} | "
        f"{'--------':>10} | {delta:10.6f} | {'Y':>5} |",
        flush=True,
    )
    ...
    while (force_max > tol_max or drm_force > tol_drm) and it < max_iter:
        ...
        coordinates_bohr, forces_Ha_bohr, energy_Ha, hessian, delta, accepted, info = (
            bfgs_step_coord_internal(
                path=geo_opt_path,
                name=name,
                atoms=atoms,
        ...
        if info["step_type"] == "Trust":
            ...
            if stall_counter >= stall_patience:
                if not hessian_reset_active:
                    hessian = (
                        []
                    )  # Reset Hessian, it will be re-initialized in the next step
            ...
            print(
                f"{it:4d} | {energy_Ha:12.8f} | {energy_Ha - energy_Ha_prev:12.8f} | {info.get('pred', '--------'):12.8f} | "
                ...
                f"{'Y' if accepted else 'N':>5} |{info.get('step_type','Trust'):>8} |",
                flush=True,
            )
        else:
            stall_counter = 0
    ...
    if force_max <= tol_max and drm_force <= tol_drm:
        ...
        print(
            f"Optimization stopped after reaching the maximum of {max_iter} iterations."
        )
    else:
...
def bfgs_step_coord_internal(
    ...
    q_k, B_prim = cartesian_to_internal_coordinates(
        np.array(coordinates_bohr), bonds, angles, dihedrals, impropers
    )
    B, U = get_B_non_redundant_internal(B_prim)
    B_plus_T = np.linalg.pinv(B, rcond=PINV_RCOND_TOL).T
    projection_error = np.max(
        np.abs(forces_vec_cart_k - (B.T @ B_plus_T) @ forces_vec_cart_k)
    )
    ...
    if len(hessian) == 0:
        ...
        hessian = get_initial_Hessian(
            atoms, coordinates_bohr, bonds, angles, dihedrals, impropers, s=1
        )
    ...
    if hessian_reset_active or fmax < 1.2e-3:
        ...
        with open(file_to_update, "w") as f:
            ...
            for atom, coord in zip(atoms, coords_trial):
                f.write(
                    f"{atom:2s} {coord[0]*ConversionFactors['a.u.->A']:20.12f} "
        ...
        lst_sq_matrix = np.array(
            [
                [delta_x**4, delta_x**3, delta_x**2],
                [3**4 * delta_x**4, 3**3 * delta_x**3, 3**2 * delta_x**2],
                [4 * 3**3 * delta_x**3, 3**3 * delta_x**2, 2 * 3 * delta_x],
                [4 * delta_x**3, 3 * delta_x**2, 2 * delta_x],
            ]
        )
        lst_sq_vector = np.array(
            [E1 - E0 - d * delta_x, E2 - E0 - 2 * d * delta_x, g2 - d, g1 - d]
        )
        ...
        q_k_plus_1, B_prim_new = cartesian_to_internal_coordinates(
            coordinates_new_bohr, bonds, angles, dihedrals, impropers
        )
        ...
        for i in range(
            dihedral_angle_start_index, len(bonds) + len(angles) + len(dihedrals)
        ):
        ...
        dg_new = -B_plus_new_T @ (
            np.array(forces_new_Ha_bohr).flatten() - np.array(forces_Ha_bohr).flatten()
        )
    ...
    with open(file_to_update, "w") as f:
        ...
        for atom, coord in zip(atoms, coordinates_new_bohr):
            f.write(
                f"{atom:2s} {coord[0]*ConversionFactors['a.u.->A']:20.12f} {coord[1]*ConversionFactors['a.u.->A']:20.12f} {coord[2]*ConversionFactors['a.u.->A']:20.12f}\n"
            )
    ...
    rho = ared / pred
    delta_new, accepted = update_trust_rad(
        rho=rho, delta=delta, delta_min=delta_min, delta_max=delta_max
    )
    ...
    if accepted:
        ...
        if np.shape(U_new) == np.shape(U):
            dg_new = -B_plus_new_T @ (
                np.array(forces_new_Ha_bohr).flatten()
                - np.array(forces_Ha_bohr).flatten()
            )
        else:
...
def bfgs_update(
    dx: np.ndarray, dg: np.ndarray, hess_mat: np.ndarray, min_curv: float = 0.2
) -> np.ndarray:
    """
    ...
    hess_mat += (
        np.outer(dg_damped, dg_damped) / np.dot(dg_damped, dx)
...
def update_trust_rad(
    rho: float, delta: float, delta_min: float, delta_max: float
) -> float:
    """
...
def get_K_bond(atom1, atom2, r1_bohr, r2_bohr, m=4.5):
    ...
    Kr_Ha_bohr2 = (
        conversion_factor_kcal_mol_A2_Ha_bohr2
        * np.exp(ln_Kij)
        / (np.linalg.norm(r1_A - r2_A)) ** (m)
    )
    return Kr_Ha_bohr2
...
C_Values = {"C": 1.339, "N": 1.300, "O": 1.249, "P": 0.906, "S": 1.448}
Z_Values = {
    "H": 0.784,
    "C": 1.183,
    "N": 1.212,
...
def get_initial_Hessian(
    ...
    for it_bond, bond in enumerate(bonds):
        i_idx = bond[0]
        j_idx = bond[1]
        H_int_diag[it_bond] = get_K_bond(
            atoms[i_idx], atoms[j_idx], coordinates_bohr[i_idx], coordinates_bohr[j_idx]
        )
    ...
    for it_angle, angle in enumerate(angles):
        ...
        H_int_diag[len(bonds) + it_angle] = get_K_theta(
            atoms[i_idx], atoms[j_idx], atoms[k_idx], bond_order1, bond_order2
        )
...
def prepare_force_eval(path, config):
    ...
    with open(xyz_path, "w") as f:
        ...
        for atom, coord in zip(atoms, coordinates):
            f.write(
                f"{atom:2s} {coord[0]:20.12f} {coord[1]:20.12f} {coord[2]:20.12f}\n"
            )

    generate_input(config=config, write_path=path, accuracy="fine")
...
def run_energy_force_stress_eval(
    ...
    with open(output_path, "w") as out_f:
        process = subprocess.Popen(
            cmd, cwd=path, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True
        )
...
def parse_cp2k_config(config) -> dict:
    ...
    if len(parts) < 4:
        raise ValueError(
            "Expected at least 4 parts: name; run_mode@xc; basis/file; potential/file"
        )
    ...
    if "@" not in parts[1]:
        raise ValueError(
            "Second part must contain '@' separating run_mode and xc_functional"
        )
...
def generate_input(config, write_path="./", accuracy="fine"):
    """
    ...
    if config["periodic"] == "None":
        string += dft_basis_set_section(
            basis_set_file=config["basis_file"], potential_file=config["potential_file"]
        )
    else:
        string += dft_basis_set_section(
            basis_set_file=config["basis_file"],
            potential_file=config["potential_file"],
            stress_tensor="ANALYTICAL",
        )
    string += dft_cutoff_rel_cutoff(accuracy=accuracy)

    hybrid = config["xc_functional"].split("_")[0] in [
        "CAM-B3LYP",
        "LCwPBE",
        "B3LYP",
        "PBEO",
    ...
    string += qs_section(
        method="GPW", accuracy=accuracy, extrapolation="ASPC", hybrid=hybrid
    )

    # Charge / multiplicity logic
    if config["UKS"]:
        string += dft_charge_multiplicity(
            charge=config["charge"], multiplicity=config["multiplicity"], LSD=True
        )
    else:
        ...
        if (num_of_electrons - int(config["charge"])) % 2 != 0:
            ...
            string += dft_charge_multiplicity(
                charge=config["charge"], multiplicity=config["multiplicity"], LSD=False
            )
    cell = config["cell_A"]
    # Cell / periodicity
    ...
    if config["run_mode"] == "geo" or config["run_mode"] == "force":
        string += force_stress_eval_print_section(
            forces_on=True, forces_filename="=Forces", ndigits=15
        )
...
def global_section(molecule_name, run_mode="energy"):
    ...
    if run_mode == "energy":
        ...
        raise ValueError(
            f"Unsupported run_mode: '{run_mode}'. Choose from 'energy', 'va', 'force', 'cell', or 'geo'."
        )
...
def qs_section(method="GPW", accuracy="fine", extrapolation="ASPC", hybrid=False):
    """
...
def geo_opt_section(accuracy="fine", optimizer="BFGS"):
    """
...
def dft_basis_set_section(
    basis_set_file="HFX_BASIS", potential_file="GTH_POTENTIALS", stress_tensor="NONE"
):
    """
...
def scf_section(
    ...
    if method.upper() == "OT":
        ...
        raise ValueError(
            f"Unsupported SCF method '{method}'. Choose 'OT' or 'STANDARD'."
        )
...
def data_for_force_field(path="./", cluster="local"):
    ...
    if not os.path.exists(start_path):
        ...
        q0, B_prim = cartesian_to_internal_coordinates(
            np.array(coordinates_bohr), bonds, angles, dihedrals, impropers
        )
    ...
    for it, bond in enumerate(bonds):
        for it in range(1, 3):
            ...
            _, B_current = cartesian_to_internal_coordinates(
                np.array(coordinates_bohr), bonds, angles, dihedrals, impropers
            )
