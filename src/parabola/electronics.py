    ...
    for idx1, idx2 in zip(nonzero_indices[0], nonzero_indices[1]):
        ...
        for l in l_ordering[atoms[idx1]]:
            ...
            basis_transformation[
                start_index_1 : start_index_1 + np.shape(block)[0],
                start_index_2 : start_index_2 + np.shape(block)[1],
            ] = block
        ...
        if len(mol.electronics.real_eigenstates.keys()) == 1:
            ...
            for sym_sector1 in other_sym_sectors.keys():
                for sym_sector2 in gamma_sym_sectors.keys():
                    OLP = np.dot(
                        other_sym_sectors[sym_sector1].T,
                        np.dot(phase_1, gamma_sym_sectors[sym_sector2]),
                    )
            ...
            for sym_sector in non_prim_gam_sym:
                for it, state in enumerate(
                    ...
                    if np.abs(value) > 0.25:
                        print(
                            np.array([str(sym_sector), str(it)]),
                            value,
                            np.abs(value) * np.sqrt(2),
                            state.energy - s0.energy,
                        )
