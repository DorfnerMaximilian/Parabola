...
mol = p.structure.Molecular_Structure(name="cofcc", path=calcpath, electronics_path=calcpath)
...
for number in range(-5, 5):
    ...
    print("ENERGY:\t", mol.Electronics.energies["alpha"])
    this_state_en = mol.Electronics.energies["alpha"][this_state[0]][int(this_state[1])].energy
