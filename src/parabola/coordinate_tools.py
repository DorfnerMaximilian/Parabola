...
def get_inertia_tensor(coordinates, masses):
    ...
    if len(coordinates[0]) == 3:
        ...
        centerofmasscoordinates, _ = compute_center_of_mass_coordinates(
            coordinates, masses
        )
...
def get_principle_axis(centeredCoordinates, masses=None, tol=1e-3):
    ...
    def is_degenerate(evals, threshold=tol):
        ...
        for i in range(len(evals)):
            for j in range(i + 1, len(evals)):
                if (
                    np.abs(evals[i] - evals[j]) / np.mean([evals[i], evals[j]])
                    < threshold
                ):
                    pairs.append((i, j))
        if not pairs:
    ...
    if len(centeredCoordinates[0]) == 3:
        ...
        for p in [[0, 1, 2], [1, 0, 2], [0, 2, 1], [2, 1, 0], [1, 2, 0], [2, 0, 1]]:
            ...
            current_overlap = (
                overlap_matrix[p[0], 0]  # old axis p[0] with new x
                + overlap_matrix[p[1], 1]  # old axis p[1] with new y
                + overlap_matrix[p[2], 2]
            )  # old axis p[2] with new z
...
def get_principle_axis_coordinates(xyzcoordinates, masses=None, axis=None):
    ...
    if len(xyzcoordinates[0]) == 3:
        if axis is None:
            if masses is not None:
                centered_coordinates, center = compute_center_of_mass_coordinates(
                    xyzcoordinates, masses
                )
                v1, v2, v3 = get_principle_axis(centered_coordinates, masses)
            else:
                centered_coordinates, center = compute_center_of_geometry_coordinates(
                    xyzcoordinates
                )
        ...
        for coordinate in centered_coordinates:
            ...
            principleaxiscoordinates.append(
                np.array([v1coordinate, v2coordinate, v3coordinate])
            )
...
def generate_internal_representation(
    ...
    List[Tuple[int, int]], List[Tuple[int, int, int]], List[Tuple[int, int, int, int]]
]:
    """
...
def get_bonds(
    ...
    # Convert dictionary
    covalent_radii_bohr = {
        atom: r * ANGSTROM_TO_BOHR for atom, r in covalent_radii_angstrom.items()
    }
    ...
    for i in range(n_atoms):
        for j in range(i + 1, n_atoms):
            try:
                r_sum = (
                    covalent_radii_bohr[atomic_symbols[i]]
                    + covalent_radii_bohr[atomic_symbols[j]]
                    + tol
                )
...
def assign_bond_orders(atoms, bonds):
    ...
    atom_data = [
        {
            "symbol": atoms[i],
            "neighbors": [],
            "total_valence": TYPICAL_VALENCE.get(
                atoms[i], 0
    ...
    while progress:
        ...
        sorted_indices = sorted(
            ...
            key=lambda i: sum(
                1
                for n in atom_data[i]["neighbors"]
                if bond_order_map[tuple(sorted((i, n)))] is None
            ),
        )
        ...
        for idx in sorted_indices:
            ...
            # or the two N-O bonds in a nitro group.
                ...
                is_symmetric = all(
                    atom_data[n]["symbol"] == first_neighbor_symbol
                    for n in unknown_bonds_to
                )
...
def get_impropers(
    n_atoms: int, adj: Dict[int, List[int]]
...
def cartesian_to_internal_coordinates(coordinates, bonds, angles, dihedrals, impropers):
    ...
    for it, dihedral in enumerate(dihedrals):
        ...
        B_o = (
            (r_u * cos_theta_u - r_w) / (r_u * r_w * sin_sq_theta_u)
        ) * normal_mop + (cos_theta_v / (r_w * sin_sq_theta_v)) * normal_opn

        B_p = (
            (r_v * cos_theta_v - r_w) / (r_v * r_w * sin_sq_theta_v)
        ) * normal_opn + (cos_theta_u / (r_w * sin_sq_theta_u)) * normal_mop
        ...
        Bq[row_idx, 3 * o_idx : 3 * o_idx + 3] = (
            -(B_m + B_p + B_n) * scale_dihedrals
        )  # B_o from formula above
    ...
    for it, improper in enumerate(impropers):
        ...
        for xyz_it in [0, 1, 2]:
            ...
            Bq[it + len(bonds) + len(angles) + len(dihedrals), i_index] = grad_r1_q[
                xyz_it
            ]
            Bq[it + len(bonds) + len(angles) + len(dihedrals), j_index] = grad_r2_q[
                xyz_it
            ]
            Bq[it + len(bonds) + len(angles) + len(dihedrals), k_index] = grad_r3_q[
                xyz_it
            ]
            Bq[it + len(bonds) + len(angles) + len(dihedrals), l_index] = grad_r4_q[
                xyz_it
            ]
...
def backtransform_iterative_refinement(
    ...
    q1_prim, _ = cartesian_to_internal_coordinates(
        x1.reshape((natoms, 3)), bonds, angles, dihedrals
    )
    dq1_prim = q1_prim - q0_prim
    dq1_prim = _wrap_primitive_dq(
        dq1_prim, n_bonds, n_angles, n_dihedrals, radians=angles_in_radians
    )
    dq1_nr = U.T @ dq1_prim
    ...
    if verbose:
        print(
            f"Back-transform refinement: initial ||Δq1|| = {norm_dq1:.3e}, max_iter = {max_iter}"
        )
    ...
    for k in range(1, max_iter + 1):
        ...
        qk_prim, _ = cartesian_to_internal_coordinates(
            x.reshape((natoms, 3)),
            bonds,
            angles,
            dihedrals,
        ...
        achieved_prim = _wrap_primitive_dq(
            achieved_prim, n_bonds, n_angles, n_dihedrals, radians=angles_in_radians
        )
        ...
        if rms <= rms_thresh:
            if verbose:
                print(
                    f"Converged in {k} iterations (RMS {rms:.3e} ≤ {rms_thresh:.3e})."
                )
            return (x + dx).copy().reshape(-1)
        if prev_rms is not None and abs(prev_rms - rms) <= rms_change_thresh:
            if verbose:
                print(
                    f"Stalled at iter {k} (RMS change ≤ {rms_change_thresh:.1e}). Returning best geometry."
                )
        ...
        if norm_delta_q > norm_dq1 and verbose:
            print(
                f"[iter {k}] ||Δq_k|| ({norm_delta_q:.3e}) > ||Δq_1|| ({norm_dq1:.3e}) - continuing but best may be x1"
            )
    ...
    if verbose:
        print(
            f"Max iterations reached ({max_iter}). Returning best geometry with ||Δq|| = {best_norm:.3e}."
        )
    return best_x.copy().reshape(-1)
