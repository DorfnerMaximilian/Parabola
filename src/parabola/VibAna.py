...
def Vib_Ana_inputs(
    ...
    os.system(
        "cp " + parentpath + inpfilename + " " + parentpath + "Equilibrium_Geometry"
    )
    os.system(
    ...
    for it in range(len(vectors)):
        ...
        for sign in [0, 1]:
            ...
            if cartesianflag:
                ...
                delta = deltas[it]
            Geometry.changeConfiguration(
                folderlabel=folderlabel,
                vector=vec,
                delta=delta * ConversionFactors["a.u.->A"],
                sign=sign,
                path_xyz=parentpath,
                path_to=parentpath,
            )
...
def CheckinpfileforVib_Ana(parentpath):
    ...
    if len(inp_files) != 1:
        raise ValueError(
            "InputError: There should be only one .inp file in the "
            + parentpath
            + " directory"
        )
            ...
            if len(line.split()) > 0:
                ...
                if line.split()[0] == "&AO_MATRICES":
                    if len(line.split()) > 1 and line.split()[1] != "ON":
                        ValueError(
                            "Reconsider the AO_Section in the .inp file! Write '&AO_MATRICES ON' !"
                        )
                    else:
                        AOSection = True
                if line.split()[0] == "&FORCES":
                    if len(line.split()) > 1 and line.split()[1] != "ON":
                        ValueError(
                            "Reconsider the &FORCES in the .inp file! Write '&FORCES ON' !"
                        )
                    else:
                        ForcesSection = True
                if line.split()[0] == "&SCF":
                ...
                if AOSection:
                    if len(line.split()) > 1:
                        if (
                            line.split()[0] == "KOHN_SHAM_MATRIX"
                            and line.split()[1] == ".TRUE."
                        ):
                            KS_Flag = True
                        ...
                        if (
                            ...
                            and line.split()[1] == "=KSHamiltonian"
                        ):
                ...
                if ForcesSection:
                    if len(line.split()) > 1:
                        ...
                        if (
                            line.split()[0] == "FILENAME"
                            and line.split()[1] == "=Forces"
                        ):
                            Forces_FilenameFlag = True
    if not KS_Flag:
        ValueError(
            "Reconsider the AO_Section in the .inp file! Write 'KOHN_SHAM_MATRIX .TRUE.' !"
        )
    if not NDIGITSKSFlag:
        ValueError("Reconsider the AO_Section in the .inp file! Write 'NDIGITS 15' !")
    if not OL_Flag:
        ValueError(
            "Reconsider the AO_Section in the .inp file! Write 'OVERLAP .TRUE.' !"
        )
    if not KS_FilenameFlag:
        ValueError(
            "Reconsider the AO_Section in the .inp file! Write 'FILENAME =KSHamiltonian' !"
        )
    if not RestartFlag:
        ValueError("Reconsider the SCF Section in the .inp file! Print Restart files !")
    if not NDIGITSForcesFlag:
        ValueError(
            "Reconsider the FORCES Section in the .inp file! Write 'NDIGITS 15' !"
        )
    if not Forces_FilenameFlag:
        ValueError(
            "Reconsider the AO_Section in the .inp file! Write 'FILENAME =Forces' !"
        )
...
def deflectAlongModes(parentfolder="./"):
    ...
    vectors = []
    deltas = []
    vectors.append(trans_vec[0, :])
    vectors.append(trans_vec[1, :])
    vectors.append(trans_vec[2, :])
    deltas.append(0.1)
    deltas.append(0.1)
    deltas.append(0.1)
    if not Read.read_periodicity(parentfolder):
        vectors.append(rot_vec[0, :])
        vectors.append(rot_vec[1, :])
        vectors.append(rot_vec[2, :])
        deltas.append(0.1)
        deltas.append(0.1)
        deltas.append(0.1)
...
def get_trans_and_rot_eigenvectors(coordinates, masses):
    ...
    centerofmasscoordinates, _ = Geometry.ComputeCenterOfMassCoordinates(
        coordinates, masses
    )
...
def getHessian(parentfolder="./", writeMolFile=True):
    ...
    for it in range(3 * numofatoms):
        ...
        sqrtMm1[it][it] = 1.0 / (np.sqrt(atomicmasses[atomorder[atomnum]]))
    ...
    rescaledHessian *= (10 ** (3) / 1.8228884842645) * (2.19474631370540e02) ** 2
    ...
    threshhold_string = input(
        "Maximally allowed Weight of Translations with Numerical Normal Modes [float between 0 and 1 or std for Standard Value]:"
    )
    ...
    if strin == "N":
        ...
        threshhold_string = input(
            "Maximally allowed Weight of Rotations with Numerical Normal Modes [float between 0 and 1 or std for Standard Value]:"
        )
        if Util.is_number(threshhold_string):
    ...
    Transeigenvectors = Symmetry.getTransEigenvectors(
        parentfolder + "/Equilibrium_Geometry/", True
    )
    Roteigenvectors = Symmetry.getRotEigenvectors(
        parentfolder + "/Equilibrium_Geometry/", True
    )
    ...
    for it in range(3 * numofatoms):
        ...
        weight_trans = np.abs(
            np.dot(
                Normalmodes[:, it],
                np.dot(Orthogonalprojector_trans, Normalmodes[:, it]),
            )
        )
        if Rotations_Projector_Flag:
            ...
            weight_rot = np.abs(
                np.dot(
                    Normalmodes[:, it],
                    np.dot(Orthogonalprojector_rot, Normalmodes[:, it]),
                )
            )
            if weight_rot > threshhold_rot and weight_trans > threshhold_trans:
                ...
                print(
                    ...
                    + str(it)
                    + " with Frequency="
                    + str(np.sign(Lambda[it]) * np.sqrt(np.abs(Lambda[it])))
                    + " 1/cm "
                    + "has rot weight "
                    + str(1.0 - weight_rot)
                )
            ...
            if weight_trans > threshhold_trans:
                orthogonalized_Vibrations.append(
                    projection_trans / normofprojection_trans
                )
                Vibrational_eigenvalues.append(Lambda[it])
            else:
                print(
    ...
    normalmodeEnergies_preliminary = (
        np.array(Vibrational_eigenvalues)
        * np.sqrt(np.abs(np.array(Vibrational_eigenvalues)))
        / np.abs(np.array(Vibrational_eigenvalues))
    )
    ...
    if len(translational_subspace) != 3:
        print(
            "Translational Subspace has Dimension "
            + str(len(translational_subspace))
            + "!"
        )
        exit()
    if Rotations_Projector_Flag:
        if len(rotational_subspace) != 3:
            print(
                "Rotational Subspace has Dimension "
                + str(len(rotational_subspace))
                + "!"
            )
            exit()
    normalmodes = []
    ...
    np.save(
        parentfolder + "/normalized-Carthesian-Displacements", carthesianDisplacements
    )
    np.save(parentfolder + "/Norm-Factors", normfactors)
    Transeigenvectors_unscaled = Symmetry.getTransEigenvectors(
        parentfolder + "/Equilibrium_Geometry/", False
    )
    Roteigenvectors_unscaled = Symmetry.getRotEigenvectors(
        parentfolder + "/Equilibrium_Geometry/", False
    )
    ...
    if writeMolFile:
        Write.write_mol_file(
            normalmodeenergies, carthesianDisplacements, normfactors, parentfolder
        )
...
def CorrectNormalModeEnergies_Input(delta=0.1, parentfolder="./"):
    ...
    normalmodeenergies, normalizedcarthesiandisplacements, _ = Read.read_vibrations(
        parentfolder
    )
    vectorsToCorrect = []
    ...
    for it in vectorsToCorrect:
        ...
        if Projectname == "emptyString":
            raise ValueError("InputError: Projectname not found!")
        Restart_files = [
            f for f in os.listdir(parentfolder) if f.endswith("-RESTART.wfn")
        ]
        if len(Restart_files) != 1:
            raise ValueError(
                "InputError: There should be only one Restart file in the current directory"
            )
        Restart_filename = Restart_files[0]
        if Restart_filename != Projectname + "-RESTART.wfn":
            raise ValueError(
        ...
        fo = [
            f
            for f in os.listdir(parentfolder + "/" + "Correction_Calc/")
            if f == foldername
        ]
        if len(fo) == 0:
        ...
        for displacementnumber in range(1, 2):
            for sign in [0, 1]:
                ...
                if len(fo) == 0:
                    Geometry.changeConfiguration(
                        str(displacementnumber),
                        normalizedCartesianDisplacement,
                        displacementnumber * delta * ConversionFactors["a.u.->A"],
                        sign,
                        parentfolder,
                        folderpath,
                    )
                    ...
                    os.system(
                        "ln -s " + pathtobinaries + "/" + "cp2k.popt" + " " + work_dir
                    )


def CorrectNormalModeEnergies_Output(
    delta=0.1, path_to_original_data="./", path_to_correctiondata="./Correction_Calc/"
):
    """
    ...
    """
    VibrationalFrequencies, NormCarthesianDisplacements, normfactors = (
        Read.read_vibrations(path_to_original_data)
    )
    ...
    for overdir in os.listdir(path_to_correctiondata):
        ...
        OLP = np.zeros(
            (len(NormCarthesianDisplacements) - 1, len(NormCarthesianDisplacements) - 1)
        )
        iterator1 = 0
        iterator2 = 0
        ...
        subfolders_in_folders = [
            x[0] for x in os.walk(path_to_correctiondata + "/" + overdir)
        ][2:]
        displacements = []
        Forces = []
        ...
        for dir in subfolders_in_folders:
            ...
            Forces.append(
                np.dot(
                    NormCarthesianDisplacements[itmode],
                    np.dot(Orthogonalprojector, Force),
                )
            )
        ...
        for it, displacement in enumerate(displacements):
            if np.abs(displacement - delta) < 10 ** (-12):
                ...
                prefactor = 0.0
            dEbydXsquared += (
                prefactor * Forces[it] * 8.2387234983e-8 / (5.29177210903 * 10 ** (-11))
            )
        ...
        Energy = (
            1.05457182
            * 10 ** (-34)
            * np.sign(dEbydXsquared)
            * np.sqrt(np.abs(dEbydXsquared) / 1.660539)
            * normfactors[itmode]
            * 10 ** (13.5)
            / (1.602 * 10 ** (-22))
        )
    ...
    np.save(
        "normalized-Carthesian-Displacements",
        NormCarthesianDisplacements[sortedIndices],
    )
    np.save("Norm-Factors", normfactors[sortedIndices])
    # Write a Mole file with corrected data
    Write.write_mol_file(
        VibrationalFrequencies[sortedIndices],
        NormCarthesianDisplacements[sortedIndices],
        normfactors[sortedIndices],
...
def get_ir_transition_dipole_moments(parentfolder="./"):
    ...
    for lambd in range(len(normfactors)):
        ...
        # Project out the Force components into direction of rotation and translation D.O.F.
        try:
            GS_DPM_Plus = Read.read_GS_Dipole_Moment(
                parentfolder + "/" + folderplus + "/GS_DIPOLE_MOMENTS"
            )
        except:
            print("Error in folder: " + parentfolder + "/" + folderplus)
            exit()
        try:
            GS_DPM_Minus = Read.read_GS_Dipole_Moment(
                parentfolder + "/" + folderminus + "/GS_DIPOLE_MOMENTS"
            )
        except:
            print("Error in folder: " + parentfolder + "/" + folderminus)
            exit()
        ir_TDM = (
            (GS_DPM_Plus - GS_DPM_Minus)
            / (2 * deltas[lambd])
            / normfactors[lambd]
            / np.sqrt(
                (
                    2
                    * ConversionFactors["u->a.u."]
                    * ConversionFactors["1/cm->a.u."]
                    * VibrationalFrequencies[lambd]
                )
            )
        )
    ...
    with open(parentfolder + "/" + "IR-Transition-Dipolemoments.dat", "w") as file:
        ...
        for it in range(len(normfactors)):
            file.write(
                format(it + 1, "5.0f")
                + format(VibrationalFrequencies[it], "12.6f")
                + format(IR_TDMs[it][0], "12.6f")
                + format(IR_TDMs[it][1], "12.6f")
                + format(IR_TDMs[it][2], "12.6f")
                + "\n"
            )
