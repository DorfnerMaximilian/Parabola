...
def get_xyz_filename(path="./", verbose=True):
    ...
    if len(xyz_files) == 1:
        ...
        raise ValueError(
            f"AmbiguityError: Found {len(xyz_files)} standard '*.xyz' files in '{path}'. Please keep only one."
        )
...
def get_cell_filename(path="./", verbose=True):
    ...
    if len(opt_files) == 1:
        ...
        raise ValueError(
            f"AmbiguityError: Found {len(opt_files)} '*_opt.cell' files in '{path}'. Please keep only one."
        )
    ...
    if len(cell_files) == 1:
        ...
        raise ValueError(
            f"AmbiguityError: Found {len(cell_files)} standard '*.cell' files in '{path}'. Please keep only one."
        )
    ...
    with open(parentfolder + "/MOs") as f:
        ...
        for line in f:
            if len(line.split()) > 6:
                if (
                    line.split()[0] == "MO|"
                    and line.split()[1] == "EIGENVALUES,"
                    and line.split()[2] == "OCCUPATION"
                    and line.split()[3] == "NUMBERS,"
                    and line.split()[4] == "AND"
                    and line.split()[5] == "SPHERICAL"
                    and line.split()[6] == "EIGENVECTORS"
                ):
                ...
                if (
                    ...
                    if abs(float(line.split()[2])) >= minweight:
                        stateiterator.append(
                            [
                                int(line.split()[0]) - homoid - 1,
                                int(line.split()[1]) - homoid - 1,
                                float(line.split()[2]),
                            ]
                        )
                if len(line.split()) == 3:
                    if (
                        represents_int(line.split()[0])
                        and not represents_int(line.split()[1])
                        and line.split()[2] == "eV"
                    ):
        ...
        for line in f:
            if len(line.split()) > 4:
                ...
                if (
                    ...
                    readflag = True
    np.save(path + "/" + "transition_dipole_vectors", TransitionDipolevectors)
    np.save(path + "/" + "oscillatorstrengths", Oscillatorstrenghts)
