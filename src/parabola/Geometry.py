...
def getPrincipleAxisCoordinates(xyzcoordinates, masses=None, axis=None):
    ...
    if axis is None:
        if masses is not None:
            centered_coordinates, _ = ComputeCenterOfMassCoordinates(
...
def get_Geometric_Principle_Axis_Coordinates(path="./"):
    ...
    Write.write_xyz_file(
        atomic_symbols=atomic_symbols,
        coordinates=principleaxiscoordinates,
    ...
    for itx in range(-m, m + 1, 1):
        for ity in range(-n, n + 1, 1):
            for itz in range(-l, l + 1, 1):
                vectortoappend = (
                    itx * cell[0] + ity * cell[1] + itz * cell[2]
...
def centerMolecule(path=None):
    ...
    for cellvector1 in cellcoordinates:
        for cellvector2 in cellcoordinates:
            ...
            if absscalarproduct > 1e-10 and abscrossproduct > 1e-10:
                raise ValueError(
                    "Centering of molecule requires orthorhombic unit cells. Use orthogonal vectors."
                )

    # Compute center of cell (assuming orthogonal basis vectors)
    cellcenter = (
        0.5 * cellcoordinates[0] + 0.5 * cellcoordinates[1] + 0.5 * cellcoordinates[2]
    )
    ...
    if mincellsizex <= 0.5 * maxdistx:
        raise ValueError(
            f"Increase cell size in x direction to at least {2 * mincellsizex}"
        )
    if mincellsizey <= 0.5 * maxdisty:
        raise ValueError(
            f"Increase cell size in y direction to at least {2 * mincellsizey}"
        )
    if mincellsizez <= 0.5 * maxdistz:
        raise ValueError(
            f"Increase cell size in z direction to at least {2 * mincellsizez}"
        )
    Write.write_xyz_file(
        atomicsym,
        centerofcellCoordinates,
        ...
        comment=comment,
        overwrite=True,
        success_comment="Successfully centered coordinates in path",
    )
...
def changeConfiguration2(
    ...
    xyzcoordinates = (
        xyzcoordinates
        + (-1) ** (sign1) * delta1 * vector1
        + (-1) ** (sign2) * delta2 * vector2
    )
    symbolsign1 = "+"
    symbolsign2 = "+"
    if sign1 == 0:
...
def getNewXYZ(path="."):
    ...
    if len(xyz_files) != 1:
        raise ValueError(
            "InputError: There should be only one old xyz file in the current directory"
        )
